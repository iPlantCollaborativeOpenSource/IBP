              ...
              function() {
                ...
                $.ajax({
                  ...
                  success: function(data, status, xmlReq){
                      if (data.success) {
                        var reply = $("<div>").html(data.body), replies;
                        if (data.mode === 'flat') {
                          replies = form.parents('.teaser-comments');
                        } else { // threaded
                          replies = form.parents('.teaser-comment').next('div.indented');
                          if (replies.length === 0) {
                        ...
                        reply.hide().fadeIn();
                        if (data.sort == 'oldest') {
                          replies.find('.box').before(reply.html());
                        } else {
                ...
                $.ajax({
                  ...
                  success: function(data, status, xmlReq) {
                    if (data.success) {
                      ...
                      if (CKEDITOR) {
                        for (var instance in CKEDITOR.instances) {
                          CKEDITOR.instances[instance].setData(data.body);
                        }
                      }
                    } else {
                      form.find('textarea').parent().addClass('error').append('<div class="error-message">' + data.body + '</div>');
