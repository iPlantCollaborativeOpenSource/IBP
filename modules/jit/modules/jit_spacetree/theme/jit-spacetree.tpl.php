	...
	<?php if ($options['enable_full_screen'] || $options['enable_search'] || $options['enable_hiding']) : ?>
		...
		<?php if ($options['enable_search']) : ?>
				...
				<?php if ($options['search_path']) { ?>
					<div class="spacetree-search-form"><?php echo drupal_get_form('jit_spacetree_node_search_form', $options['search_path']); ?></div>
				<?php } else if (variable_get('jit_enable_debug', 0)) { ?>
...
<script type="text/javascript">
	Drupal.behaviors.jit_spacetree_<?php echo $options['id']; ?> = function (context) {
		if (! window.jit_spacetree) {
			window.jit_spacetree = {};
		}
		if (! window.jit_spacetree['<?php echo $options['id']; ?>']) {
			window.jit_spacetree['<?php echo $options['id']; ?>'] = new Drupal.jit.spacetree(<?php echo drupal_to_js($options); ?>);
	
			<?php if ($controller['onBeforeCompute']) : ?>
				window.jit_spacetree['<?php echo $options['id']; ?>'].st.config.onBeforeCompute = <?php echo $controller['onBeforeCompute']; ?>;
			<?php endif; ?>
			
			<?php if ($controller['onAfterCompute']) : ?>
				window.jit_spacetree['<?php echo $options['id']; ?>'].st.config.onAfterCompute = <?php echo $controller['onAfterCompute']; ?>;
			<?php endif; ?>
		
			<?php if ($controller['onCreateLabel']) : ?>
				window.jit_spacetree['<?php echo $options['id']; ?>'].st.config.onCreateLabel = <?php echo $controller['onCreateLabel']; ?>;
			<?php endif; ?>
		
			<?php if ($controller['onPlaceLabel']) : ?>
				window.jit_spacetree['<?php echo $options['id']; ?>'].st.config.onPlaceLabel = <?php echo $controller['onPlaceLabel']; ?>;
			<?php endif; ?>
		
			<?php if ($controller['onComplete']) : ?>
				window.jit_spacetree['<?php echo $options['id']; ?>'].st.config.onComplete = <?php echo $controller['onComplete']; ?>;
			<?php endif; ?>
		
			<?php if ($controller['onBeforePlotLine']) : ?>
				window.jit_spacetree['<?php echo $options['id']; ?>'].st.config.onBeforePlotLine = <?php echo $controller['onBeforePlotLine']; ?>;
			<?php endif; ?>
		
			<?php if ($controller['onAfterPlotLine']) : ?>
				window.jit_spacetree['<?php echo $options['id']; ?>'].st.config.onAfterPlotLine = <?php echo $controller['onAfterPlotLine']; ?>;
			<?php endif; ?>
		
			<?php if ($controller['onBeforePlotNode']) : ?>
				window.jit_spacetree['<?php echo $options['id']; ?>'].st.config.onBeforePlotNode = <?php echo $controller['onBeforePlotNode']; ?>;
			<?php endif; ?>
		
			<?php if ($controller['onAfterPlotNode']) : ?>
				window.jit_spacetree['<?php echo $options['id']; ?>'].st.config.onAfterPlotNode = <?php echo $controller['onAfterPlotNode']; ?>;
			<?php endif; ?>
		
			<?php if ($controller['request']) : ?>
				window.jit_spacetree['<?php echo $options['id']; ?>'].st.config.request = <?php echo $controller['request']; ?>;
			<?php endif; ?>
			
			<?php if ($tree): ?>
				window.jit_spacetree['<?php echo $options['id']; ?>'].render(<?php echo drupal_to_js($tree); ?>);
			<?php endif; ?>
		}
	};
</script>